...
entity_float_data_change_event_handler (CManager cm, void* event, void* client_data, attr_list event_list)
{
  ...
  //FIXME:This is a bug that needs to be fixed, but it'll take too long now.  The storage stone needs to be more flexible in how we set up
  static int i = 0;
  if (i == 0)
  {
    ++i;
  ...
  pthread_mutex_lock(&fd_mutex);
  if(!log_average(new_data.data[0]))
  {
